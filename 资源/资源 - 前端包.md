...
| 包名称                    | 英文介绍                                                          | 介绍                       |
| ---------------------- | ------------------------------------------------------------- | ------------------------ |
| npm-check-updates      |                                                               | `-i` 交互模式，查看潜在的依赖升级并逐个选择 |
...
| node-modules-inspector |                                                               | 对所有依赖项进行检查，输出            |
| pnpm                   |                                                               | 替代 npm，提供 workspace      |
|                        |                                                               |                          |
|                        |                                                               |                          |
|                        |                                                               |                          |
...
| @commitlint/cli |                                      | 对 commit 信息进行 lint  |
|                 |                                      |                     |
|                 |                                      |                     |
...
| 包名称            | 英文介绍                                                                       | 介绍                      |
| -------------- | -------------------------------------------------------------------------- | ----------------------- |
| eslint         |                                                                            | 使用最广的 lint 工具，支持 js/css |
...
| jsdom                   |                                                  | 纯 JS 实现的 DOM               |
| fetch-mock              |                                                  | 请求 mock                    |
| fetch-network-simulator |                                                  | 按规则随机丢包、延迟、限速等，用来模拟糟糕网络    |
...
| rxjs                      | Reactive Extensions For JavaScript                                   | JS 的响应式扩展，链式处理                          |
| filesize                  |                                                                      | 把文件大小换算为易读的内容                           |
| @faker-js/faker           | Generate massive amounts of fake data                                | 生成大量假数据                                 |
| ip-address                |                                                                      | 解析和操作 IP v4/6 地址                        |
| fast-json-stringify       |                                                                      | 更快的 jsonstringify                       |
| destr                     |                                                                      | 实现同 `JSON.parse` 的功能但更安全，支持 ts          |
| lossless-json             |                                                                      | 避免 number 数据值过大影响 json 解析               |
...
| es-toolkit                |                                                                      | 类 lodash 工具，支持类型，性能 up                  |
...
| humanifyjs                |                                                                      | 代码反混淆                                   |
| colorthief                | Grab the color palette from an image                                 | 从图像中获取主要的配色                             |
...
| image-dimensions          |                                                                      | 获取文件信息的宽高                               |
| is-online                 |                                                                      | 用来判断网络是否真的在线                            |
| clipboardy                |                                                                      | 用来复制粘贴                                  |
|                           |                                                                      |                                         |
|                           |                                                                      |                                         |
|                           |                                                                      |                                         |
...
| ---------- | ------ | --------------------- |
| csv-parse  |        | csv 解析                |
| image-js   |        | 调整大小、裁剪、过滤、颜色调整等      |
| mediabunny |        | 媒体处理，转换（MP4）          |
| dompurify  |        | 放置 XSS 攻击，对 HTML 进行过滤 |
...
| 包名称                      | 英文介绍                                                                                                                                    | 介绍                    |
| ------------------------ | --------------------------------------------------------------------------------------------------------------------------------------- | --------------------- |
| file-type                | detected by checking the [magic number](https://en.wikipedia.org/wiki/Magic_number_(programming)#Magic_numbers_in_files) of the buffer. | 判断二进制文件的类型            |
...
| 包名称       | 英文介绍                             | 介绍               |
| --------- | -------------------------------- | ---------------- |
| wallpaper | Get or set the desktop wallpaper | 获取或者设置桌面壁纸       |
| diskusage |                                  | 跨平台磁盘使用统计        |
...
| mathlive                         |                                                                       | 数学公式的编辑和展示                     |
| copy-text-to-clipboard           |                                                                       | 将内容粘贴到剪切板（2kb）                 |
...
| @tsparticles/engine              | [https://particles.js.org/](https://particles.js.org/)                | 特效引擎                           |
...
| shadcn/ui（非 npm 包名                                          | shadcn/ui [https://ui.shadcn.com/](https://ui.shadcn.com/)                                       | 把 UI 组件生成到你的项目内，高可扩展        |
...
| react-geiger                                                | [https://github.com/kristiandupont/react-geiger](https://github.com/kristiandupont/react-geiger) | 让 React 应用卡顿时发出声音           |
| onborda                                                     |                                                                                                  | 产品导览功能（next                 |
| spectacle                                                   |                                                                                                  | 在线预览组件                      |
...
| rooks                                                       |                                                                                                  | react hooks                 |
| react-scad                                                  |                                                                                                  | 使用 react 渲染 scad 模型         |
| react-hotkeys-hook                                          |                                                                                                  | 使用键盘按键事件                    |
|                                                             |                                                                                                  |                             |
...
| @headless-tree/core         |                                                                                                          | 可拖拽的树状图组件                                                      |
| react-beautiful-color       |                                                                                                          | 浏览器内颜色拾取                                                       |
| react-window                | helps render large lists of data quickly and without the performance problems                            | 大数据渲染                                                          |
| ink                         |                                                                                                          | 使用 React 编写命令行工具                                               |
| @mantine/core    |                 |      热门组件库新增用于选择层级树数据的          |
|                             |                                                                                                          |                                                                |
|                             |                                                                                                          |                                                                |
|                             |                                                                                                          |                                                                |
|                             |                                                                                                          |                                                                |
...
| codemirror               |                                             | 代码编辑器            |
| slate                    |                                             | 编辑器框架，为构建大型应用    |
| prosemirror-state        |                                             | 编辑器框架，构建富文本编辑器首选 |
| lexical                  |                                             | 编辑器框架，react 深度集成 |
...
| markdown-it       | Markdown parser done right. Fast and easy to extend.              | markdown 解析插件                                      |        |
...
| mermaid           | definitions and a renderer to create and modify complex diagrams. | 添加更多可编辑格式                                          |        |
...
| svguitar          | 演示：[https://github.com/omnibrain/svguitar](https://github.com/omnibrain/svguitar) | 用 SVG 做吉他谱！                                                                                              |
| waveform-renderer |                                                                                   | MP3 或 WAV 文件创建可视化波形图                                                                                     |
| xmlbuilder2       |                                                                                   | JS 对象转 XML                                                                                               |
|                   |                                                                                   |                                                                                                          |
|                   |                                                                                   |                                                                                                          |
...
| fancycomponents[非包名] | [https://www.fancycomponents.dev](https://www.fancycomponents.dev) | 文字特效为主的多种动画效果            |
|                      |                                                                    |                          |
|                      |                                                                    |                          |
|                      |                                                                    |                          |
|                      |                                                                    |                          |
...
| @glideapps/glide-data-grid | [https://grid.glideapps.com/](https://grid.glideapps.com/)         | 表格 + 可视化                          |        |
| spacekit.js                |                                                                    | 宇宙 3D 视图                          |        |
| vizzu                      | [https://lib.vizzuhq.com/latest/](https://lib.vizzuhq.com/latest/) | 新的**动画**可视化库                      |        |
...
| react-native-bootsplash               |                                                          | App 启动页无缝体验       |
|                                       |                                                          |                   |
|                                       |                                                          |                   |
|                                       |                                                          |                   |
|                                       |                                                          |                   |
...
[tanstack](https://github.com/tanstack/table)，一系列的 UI **组件**，解决方案